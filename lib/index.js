...
The delegation tool in the listing above is an awaitable like any other — so a multi-agent workflow needs no separate engine. The Python IS the orchestration: \`asyncio.gather\` fans out, \`for\` loops rounds, \`if\`/\`while\` decide what survives.

Read its signature and return type in the listing before the first call. Its result is a union of branches keyed by \`kind\`, and only ONE of them carries the child's answer into this cell — the others hand you an id and settle later, which is what you get by DEFAULT where \`run_in_background\` defaults to true. Getting this wrong costs a whole round trip: you parse an id as if it were the answer. So write the two helpers once, at the top, and reuse them for every stage:

\`\`\`python
from __dsh__.tools import <the delegation tool> as delegate   # short local name
import asyncio, json

async def ask(desc, prompt):   # always foreground: this cell needs the answer
    r = await delegate(description=desc, prompt=prompt, run_in_background=False)
    return "".join(b.get("text", "") for b in r["output"] if isinstance(b, dict))

def as_json(text):   # a model asked for JSON often wraps it in a sentence
    return json.loads(text[text.index("{"):text.rindex("}") + 1])
\`\`\`

**Default to per-item chains, not stage barriers.** Write one coroutine that carries a single item through every stage, then gather the chains — so an item reaches stage 2 while another is still in stage 1:

\`\`\`python
LENSES = ["correctness", "security", "does it actually reproduce"]

async def verify(claim, lens):
    schema = '{"refuted": bool, "reason": str}'   # kept out of the f-string: doubling the braces there is easy to get wrong
    v = as_json(await ask(f"refute via {lens}", f'Refute this, through the {lens} lens: "{claim}". Default to refuted when unsure. Reply as JSON: {schema}'))
    return None if v["refuted"] else claim

async def chain(dim):
    claims = as_json(await ask(f"review {dim}", PROMPTS[dim]))["findings"]
    votes = await asyncio.gather(*(verify(c, lens) for c in claims for lens in LENSES))
    return [c for c in claims if sum(v == c for v in votes) > len(LENSES) / 2]   # majority upholds it

nested = await asyncio.gather(*(chain(d) for d in DIMENSIONS))
confirmed = [c for row in nested for c in row]
\`\`\`

Wall-clock is then the slowest single chain, not the sum of the slowest per stage. One \`gather\` per stage is the shape to avoid: it makes every fast reviewer wait on the slowest before any verification starts.

**A barrier gather is correct only when the next stage needs ALL of the previous one at once** — dedup across the whole set before expensive work, an early exit when the count is zero, a judge ranking candidates against each other. It is NOT justified by needing to flatten or filter first: do that inside the chain. When in doubt, chain.

Patterns worth knowing by name:

- **Adversarial verify** — spawn several skeptics per claim, each told to REFUTE it and to default to refuted when unsure. Keep what a majority upholds. This is what stops a plausible-but-wrong finding from surviving.
- **Perspective-diverse verify** — when a claim can fail in more than one way, give each verifier a distinct lens (correctness, security, does-it-reproduce) instead of N identical refuters. Diversity catches what redundancy cannot.
- **Judge panel** — generate N independent attempts from different angles, score them in parallel, then synthesize from the winner while grafting the best of the runners-up. Beats one-attempt-iterated when the solution space is wide.
- **Loop-until-dry** — for discovery of unknown size, keep running rounds until K consecutive rounds surface nothing new. Dedup against everything SEEN, never against what survived judging: otherwise rejected claims come back every round and it never converges.

There is no schema parameter to force a reply's shape — don't look for one. Asking for JSON in the prompt and slicing the braces out, as \`as_json\` does above, is the whole mechanism.

Scale the fan-out to what was asked: a couple of agents and one verification pass for a quick check, a wider pool and several refuters per claim for "audit this thoroughly". And if you bound coverage — top-N, one round, a sample — say so in what you report. Silent truncation reads as "covered everything".
`
...
function hasDelegationTool(schemas) {
  return schemas.some((schema) => {
    ...
    const kinds = new Set(branches.map((branch) => branch?.properties?.kind?.const).filter((kind) => typeof kind === 'string'))
    return kinds.has('foreground') && kinds.has('background') && kinds.has('continuable')
  })
}
...
export function apply(ctx, config = {}) {
  ...
  // Every constant below reaches `ctx.systemPrompt.section({text})`, and dsh-system-prompt reads `{{` there as a variable reference — a malformed one throws at ASSEMBLY, taking down every session on this preset rather than degrading one prompt. The per-tool prose inside the sdk section is escaped by `escapePromptGroups` on its way through the renderer; these hand-written literals have no such pass, so they are asserted brace-free at mount instead of being escaped at render, keeping the text the one thing a reader can trust.
  //
  // `RESTART_NOTICE` is deliberately absent: it is injected as a transcript message, which never passes through `interpolate`.
  const assertBraceFree = (label, text) => {
  ...
  // `ORCHESTRATION` and `INSTRUCTIONS` are the ones that carry Python showing JSON shapes, where an f-string's doubled literal brace is the easy mistake; `EXCLUSIVE_RULE` is asserted for the same reason it is a section at all.
  assertBraceFree('orchestration', ORCHESTRATION)
  assertBraceFree('sdk', INSTRUCTIONS)
  assertBraceFree('code-only', EXCLUSIVE_RULE)
