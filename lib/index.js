...
export function renderToolsSection(schemas) {
  ...
  const mcpBlock = servers.flatMap(([server, tools]) =>
    ...
    listing(`${TOOLS_MODULE}.mcp.${serverName(server, grouped)}`, tools.filter((t) => isUsableName(fold(t.tool))).sort(by((t) => t.tool)).map((t) => `# mcp.${serverName(server, grouped)}.${signature({ ...t, name: fold(t.tool) }).replace(/^async def /, '').replace(/: \.\.\.$/, '')}`)))
