...
function renderOutputTypes(sorted) {
  const declared = (schema) => {
    ...
    return payload === null ? schema.output : (payload ?? { type: 'string' })
  }
...
export function renderToolsSection(schemas) {
  ...
  const signature = (spec, head) => {
    ...
    //
    // `head` is checked too, and it is the same failure wearing a legal shape: `self` is a perfectly
    // good identifier, so `isUsableName` passes it, and `async def list(self, *, self: str)` is a
    // `SyntaxError: duplicate argument` that takes the WHOLE fenced block with it — every sibling
    // stub and every native signature below. Only a Protocol method spends a name this way, so at
    // the top level (`head` empty) `self` stays a real parameter.
    if (!spec.params.every((p) => isUsableName(p.name) && !head.includes(p.name))) return `async def ${spec.name}(${[...head, '**kwargs: Any'].join(', ')}) -> ${spec.returns}: ...  # not every parameter name can be spelled here; see ${spec.name}?`
  ...
  const typing = TYPING_SYMBOLS.filter((symbol) => classes.concat(mcpBlock, signatures).some((line) => new RegExp(`\\b${symbol}\\b`).test(line)))
  const lines = [
    INSTRUCTIONS,
    '',
    renderEnvironment(),
