...
- ONLY the cell's output and its final expression come back to you. Tool results consumed inside the cell never enter the conversation, so filter and aggregate in code and surface just the conclusion. Every tool answers in FULL — every field it declares, for every row — and you almost always want a few of them: bind the result and end on what you actually asked, \`r = await search(q)\` then \`[(x["url"], x["score"]) for x in r["results"]]\`. Don't know the shape yet? Ask for the SHAPE, not the payload — \`r.keys()\`, \`len(r["results"])\`, \`r["results"][0]\` — then extract on the next line.
