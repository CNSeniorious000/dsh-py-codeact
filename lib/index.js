...
r = await delegate(description="find bugs", prompt='Look for bugs. Reply as JSON: {"bugs": [{"file": str, "line": int, "issue": str}]}', run_in_background=False)
...
\`\`\`python
async def verify_claim(claim, lens):
    schema = '{"refuted": bool, "reason": str}'
...
async def chain(dim):
    ...
    votes = await asyncio.gather(*(verify_claim(c, lens) for c in claims for lens in LENSES))
    return [c for c in claims if sum(v == c for v in votes) > len(LENSES) / 2]
