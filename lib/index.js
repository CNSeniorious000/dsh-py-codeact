...
Read its signature and return type in the listing before the first call. Its result is a union of branches keyed by \`kind\`. By default it returns an id and settles the work in the background (\`continuable: true\`), which costs you a round trip if you try to parse that id as the answer. Pass \`run_in_background=False\` when this cell needs the child's output to continue, and extract what you need from the \`"output"\` field — a list of content blocks.

Asking for JSON in the prompt is the forcing mechanism. The child may wrap it in a sentence; slice the braces out:

\`\`\`python
from __dsh__.tools import <the delegation tool> as delegate
import asyncio, json

r = await delegate(description="find bugs", prompt='Look for bugs. Reply as JSON: {"bugs": [{"file": "a/b.py", "line": 12, "issue": "what is wrong"}]}', run_in_background=False)
text = "".join(b.get("text", "") for b in r["output"] if isinstance(b, dict))
result = json.loads(text[text.index("{"):text.rindex("}") + 1])
\`\`\`

Show the shape with EXAMPLE VALUES, not with type names: a child that copies \`{"line": int}\` literally hands back something \`json.loads\` refuses, and the chain dies on the parse rather than on the work.
...
\`\`\`python
async def upholds(claim, lens):   # one verdict, not the claim: votes must stay attributable
    schema = '{"refuted": true, "reason": "why"}'
    prompt = f'Refute this via the {lens} lens: "{claim}". Default to refuted when unsure. Reply as JSON: {schema}'
    r = await delegate(description=f"verify {lens}", prompt=prompt, run_in_background=False)
    text = "".join(b.get("text", "") for b in r["output"] if isinstance(b, dict))
    return not json.loads(text[text.index("{"):text.rindex("}") + 1])["refuted"]

async def chain(dim):
    r = await delegate(description=f"review {dim}", prompt=PROMPTS[dim], run_in_background=False)
    text = "".join(b.get("text", "") for b in r["output"] if isinstance(b, dict))
    claims = json.loads(text[text.index("{"):text.rindex("}") + 1])["findings"]
    LENSES = ["correctness", "security", "does it reproduce"]
    # Gather PER CLAIM, so each majority is counted over that claim's own lenses — flatten the
    # whole grid into one list and two identically-worded findings pool their votes.
    grid = await asyncio.gather(*(asyncio.gather(*(upholds(c, lens) for lens in LENSES)) for c in claims))
    return [c for c, votes in zip(claims, grid) if sum(votes) > len(LENSES) / 2]

nested = await asyncio.gather(*(chain(d) for d in DIMENSIONS))
confirmed = [c for row in nested for c in row]
\`\`\`

Wall-clock is then the slowest single chain, not the sum of the slowest per stage. One \`gather\` per stage is the shape to avoid: every fast finder waits on the slowest before any verification starts.

**A barrier is correct only when the next stage needs ALL of the previous one at once** — dedup across the whole set before expensive work, an early exit when the count is zero, ranking candidates against each other. It is NOT justified by needing to flatten or filter first: do that inside a stage. When in doubt, chain.

Patterns worth knowing:

- **Adversarial verify** — spawn several skeptics per claim, each told to REFUTE it and default to refuted when unsure. Keep what a majority upholds. This stops plausible-but-wrong findings from surviving.
...
- **Judge panel** — generate N independent attempts from different angles, score them in parallel, synthesize from the winner while grafting the best of the runners-up. Beats iterating on one attempt when the solution space is wide.
- **Loop-until-dry** — for discovery of unknown size, keep running rounds until K consecutive rounds surface nothing new. Dedup against everything SEEN, never against what survived judging — otherwise rejected claims reappear every round and it never converges.

Scale to what was asked: a couple of agents and one verification pass for a quick check, a wider pool and several refuters per claim for "audit this thoroughly". If you bound coverage — top-N, one round, a sample — say so. Silent truncation reads as "covered everything".
`
