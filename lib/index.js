...
/**
 * Whether Python would read this exactly as an identifier. Two conditions, not one: `\p{XID_*}` is the rule the language actually states, and NFKC-stability is the rule the TOKENISER adds — source is normalised before it is read, so `def f(*, ﬁ=1)` declares `fi` and `f(ﬁ=2)` binds `fi` (that is the ligature, not the two letters). Both pass `str.isidentifier()`; only the second would send a key the tool never declared, silently, so it stays folded.
 */
 ...
 */
const paramName = (raw) => {
  if (isPyIdentifier(raw)) return PY_HARD_KEYWORDS.has(raw) ? `${raw}_` : raw
  const folded = fold(raw)
  return isPyIdentifier(folded) && !PY_HARD_KEYWORDS.has(folded) ? folded : null
}
