...
export function renderToolsSection(schemas) {
  ...
  if (servers.length > 0) {
    ...
    // better than `mcp.` at every call site when a cell leans on one server.
    lines.push('', `Each server is also a module: \`from __dsh__.tools.mcp.${servers[0][0]} import ${servers[0][1].find((t) => isUsableName(t.tool))?.tool ?? '...'}\` binds its tools directly.`)
  }
