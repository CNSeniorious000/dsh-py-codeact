...
function toolSpec(schema, returns) {
  ...
  return {
    ...
    params: Object.entries(properties).map(([name, node]) => ({
      ...
      // What the parameter MEANS, which its type cannot say — `queries` is `list[str]` either way, and only the prose says 1–4 of them. Carried to the kernel rather than rendered: it is ~6.8 KB across a real catalogue, and the block would pay it every turn for the one parameter a cell actually touches. `undefined` drops out of `JSON.stringify`, so a parameter without one costs the wire and `specsKey` nothing.
...
export function renderToolsSection(schemas) {
  ...
  // a broken mount.
  lines.push('', `Each is a real function: \`name?\` shows its full description and what each parameter means, \`dir(__dsh__.tools)\` lists them${servers.length === 0 ? '' : ', and `dir(mcp)` / `dir(mcp.<server>)` the ones under `mcp`'}.`)
  return lines.join('\n')
}
