...
/** The 35 names a parameter genuinely cannot take — `keyword.kwlist`, which is 35 on every version the PEP 723 header allows (checked on 3.12, 3.13 and 3.14). The other four in {@link PY_KEYWORDS} are `keyword.softkwlist`: `_`, `case`, `match`, `type` — `def f(*, type: str)` compiles, and rejecting them cost a tool its whole signature over a parameter name as ordinary as `type`. They stay refused as TOOL names, where the block imports them and `type` would shadow the builtin for the rest of the session. (`type` only joined `softkwlist` in 3.12, which is where the header's floor is.) */
