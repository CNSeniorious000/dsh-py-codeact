...
export function renderToolsSection(schemas) {
  ...
  const signature = (spec) => {
    ...
    if (rest.length > 0) {
      ...
      // Spelled the way the named fields above are, and the way the kernel spells the same list into `<tool>?` — `'a b': str, 'c d': int = ...`. Names alone said WHICH arguments have to go through `**kwargs` but not which the tool cannot run without, so the two halves disagreed about the one thing the note exists to prevent. Deliberately not folded into `types`: this is a comment, and a `Literal` mentioned in one must not pull a `typing` import, for the same reason a description mentioning `Any` must not.
      fields.push(`    **${overflow}: Any  # spell as dict keys: ${rest.map((p) => `${JSON.stringify(p.name)}: ${p.type}${p.required ? '' : ' = ...'}`).join(', ')}`)
    }
