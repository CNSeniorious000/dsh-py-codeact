...
export function renderToolsSection(schemas) {
  ...
  const mcpSigs = servers.flatMap(([server, tools]) => tools.filter((t) => isUsableName(t.tool)).sort(by((t) => t.tool))
    .map((t) => ({ server, ...signature({ ...t, name: t.tool }) })))
  const mcpBlock = servers.flatMap(([server]) =>
    listing(`${TOOLS_MODULE}.mcp.${server}`, spaced(mcpSigs.filter((sig) => sig.server === server)
      .map((sig) => sig.lines.map((line, index) => `# ${index === 0 ? line.replace(/^async def /, `mcp.${server}.`) : line}`.trimEnd())))))
