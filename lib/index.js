...
/** The kernel's wording for an exec it answered WITHOUT taking the catalogue it carried. Matched, not parsed, exactly like {@link KERNEL_BUSY}: both are ordinary `ok: false` answers that rebound nothing, and memoising either as delivered strands the shell on a stale tool table. */
export const KERNEL_UNBOUND = 'kernel bindings failed: the tool catalogue was not taken'
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        if (result.ok || (result.error !== KERNEL_BUSY && !result.error?.startsWith(KERNEL_UNBOUND))) entry.sent.set(shell, key)
        return wasRestarted
