...
export function renderToolsSection(schemas) {
  ...
  // dsh-system-prompt has no literal `{{...}}` escape, so split every opener before tool prose enters a prompt section and gets mistaken for a registered variable.
  const escapePromptGroups = (doc) => doc.replaceAll('{{', '{ {')
  const docstring = (doc) => {
    if (!doc) return ['    ...']
