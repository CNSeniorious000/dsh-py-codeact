...
- The cell's LAST expression is echoed back to you, like a REPL prompt — that is the return channel, and reaching for it first keeps cells short. Use \`print(...)\` when you want to RESHAPE what comes back — label several values, format a table, show the FIRST few of something large — not to hand over a value the last line would have echoed anyway. A slice reshapes only while its bound is far below the payload: \`[:200]\` shows you the shape, while a bound larger than the answer changes nothing but the label.
