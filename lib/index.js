...
The delegation tool is an awaitable — multi-agent workflows need no separate engine. Pass \`run_in_background=False\` when you need the output to continue. Extract from the \`"output"\` field:

\`\`\`python
from __dsh__.tools import <the delegation tool> as delegate
import asyncio, json

def ask(prompt): return delegate(description="query", prompt=prompt, run_in_background=False)
def as_json(r): text = "".join(b.get("text", "") for b in r["output"] if isinstance(b, dict)); return json.loads(text[text.index("{"):text.rindex("}") + 1])
\`\`\`

Ask for JSON with example values, not type names — a child that copies \`int\` literally breaks \`json.loads\`.

**Per-item chains, not stage barriers.** One coroutine per item through all stages, so item A verifies while item B is still finding:

\`\`\`python
async def chain(dim):
    schema = '{"findings": ["claim1", "claim2"]}'
    findings = as_json(await ask(f'Find issues in {dim}. Reply JSON: {schema}'))["findings"]
    verdict_schema = '{"refuted": true}'
    tasks = [asyncio.create_task(ask(f'Refute: {c}. Reply JSON: {verdict_schema}')) for c in findings]
    votes = [as_json(await t) for t in asyncio.as_completed(tasks)]  # consume as ready
    return [c for c, v in zip(findings, votes) if not v.get("refuted")]

results = await asyncio.gather(*(chain(d) for d in DIMS), return_exceptions=True)
confirmed = [item for r in results if not isinstance(r, Exception) for item in r]
\`\`\`

\`return_exceptions=True\` lets one failure not kill the batch. \`as_completed\` starts verification the moment each finder returns, not after all finish. A barrier (one \`gather\` per stage) is correct only when the next stage needs ALL results at once — deduplication, early exit on zero count, cross-item ranking.
`
