...
export function renderToolsSection(schemas) {
  ...
  // Commented, because these names are NOT bound at the top level — only `mcp` is. A bare
  // `async def read(...)` under a server header claims a top-level `read` that does not exist, and
  // worse, it BINDS one: with a native `read` and two servers exposing a raw `read`, the last stub
  // wins and `await read(url=...)` then returns `None`, ok, having called NOTHING. A silent no-op
  // wearing a success, in place of the tool the first section had just imported.
  //
  // The comment body stays real Python though — `async def read_urls(`, not `mcp.web.read_urls(`.
  // The latter is not a statement in any position, so the block could only ever be read as prose;
  // the header directly above already carries the module, and with it the call path.
  ...
  const mcpBlock = servers.flatMap(([server]) =>
    listing(`${TOOLS_MODULE}.mcp.${serverName(server, grouped)}`, spaced(mcpSigs.filter((sig) => sig.server === server)
      .map((sig) => sig.lines.map((line) => `# ${line}`.trimEnd())))))
