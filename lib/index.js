...
function renderTypes(sorted) {
  ...
  // `type: 'object'` is asserted rather than assumed: dsh degrades the WHOLE args type to `Any` without it, and a schema that declares `properties` is describing an object whether or not it says so — JSON Schema has no other reading. Third trigger of the same silent degradation, after a stray `$schema` and a `oneOf` at any property; the only one a caller hits by omission rather than by content. Spread first, so a schema that DOES declare a type keeps it.
  const asObject = (parameters) => ('properties' in parameters ? { type: 'object', ...parameters } : parameters)
  const text = renderToolsSdkPy(sorted.map((schema) => ({ name: schema.name, parameters: asObject(narrowed(schema.parameters ?? {})), output: declared(schema) })))
