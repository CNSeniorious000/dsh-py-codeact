...
The delegation tool is an awaitable — Python IS the orchestration. \`asyncio.gather\` fans out, \`for\` loops rounds, \`if\`/\`while\` filters.

Its result is a union keyed by \`kind\`. By default it returns an id and runs in the background (\`continuable: true\`). Pass \`run_in_background=False\` when this cell needs the output:

\`\`\`python
from __dsh__.tools import <delegation-tool> as delegate
import asyncio, json

tasks = [("bugs", "Find correctness bugs"), ("perf", "Find performance issues")]
results = await asyncio.gather(*(
    delegate(description=f"review {name}",
            prompt=f'{desc}. Reply as JSON with "findings" array.',
...
Show example values in the schema, not type names — \`{"line": 12}\` not \`{"line": int}\`.

**Default to per-item chains, not barriers.** One coroutine per item through all stages, then gather — so item A reaches stage 2 while B is in stage 1:

\`\`\`python
async def chain(dim):
    r = await delegate(description=f"review {dim}", prompt=f"Find {dim} issues. Reply as JSON with findings array.", run_in_background=False)
    claims = json.loads("".join(b.get("text", "") for b in r["output"] if isinstance(b, dict)))["findings"]
    votes = await asyncio.gather(*(verify(c) for c in claims))
    return [c for c, v in zip(claims, votes) if v]

results = await asyncio.gather(*(chain(d) for d in ["bugs", "perf", "security"]))
\`\`\`

Wall-clock = slowest chain, not sum-of-stages. Only use a barrier when stage N needs ALL of stage N-1 at once (dedup, early exit, cross-item ranking).
`
