...
/** Identity of one binding set, for skipping a rebind that would change nothing. */
const specsKey = (specs) => specs.map((spec) => spec.name).join('\0')
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        const observation = renderCell(await entry.kernel.exec(args.code, exec.signal, rebind, shell))
        // Only once the kernel has actually taken them: a cell that never dispatched (an already-cancelled turn) would otherwise leave us believing the bindings landed, and the next cell would skip the rebind and run against a stale tool table.
        entry.sent.set(shell, key)
        return wasRestarted
