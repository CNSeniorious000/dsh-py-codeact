 ...
 */
function toolSpec(schema, declarations = new Map()) {
...
function declareType(schema, baseName, declarations) {
  ...
  if (names.length === 0 || !names.every(isUsableName)) return dedupeUnion(jsonSchemaToPy(schema))
  ...
  // Two tools whose names differ only in separators pascal-case to the same string; keep the first and let the second carry a suffix rather than silently serving one tool the other's shape.
  let name = baseName
  for (let n = 2; declarations.has(name) && declarations.get(name).join('\n') !== body.join('\n'); n++) name = `${baseName}${n}`
  declarations.set(name, [`class ${name}(TypedDict):`, ...body])
  return name
}
...
export function renderToolsSection(schemas) {
  ...
  const specs = [...schemas].sort(byName).map((schema) => toolSpec(schema, declarations))
  ...
  // The declarations are stubs, not runtime objects — nothing constructs them, and `NotRequired`
  // only ever appears inside one — so the import line lists exactly what the render used. `Any`
  // has always been reachable from a signature (`**kwargs: Any`) and was never imported.
  const body = [...importable.map(signature)].join('\n')
  const declared = [...declarations.values()].flatMap((block) => ['', ...block])
  const typing = ['Any', 'NotRequired', 'TypedDict'].filter((symbol) => (body + declared.join('\n')).includes(symbol))
  const lines = [
    INSTRUCTIONS,
    '',
    renderEnvironment(),
