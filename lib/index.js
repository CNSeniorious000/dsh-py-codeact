...
export function renderToolsSection(schemas) {
  const { specs: bound, declarations } = toolSpecs(schemas)
  // The kernel reserves `mcp` for the namespace, so a tool of that name is never bound. Rendering
  // one anyway put `mcp` in the import line twice and handed the model a signature for a call that
  // always fails.
  const specs = bound.filter((spec) => spec.name !== 'mcp')
  ...
  const servers = [...grouped].filter(([server]) => isUsableName(server)).sort(([a], [b]) => (a < b ? -1 : 1))
  ...
  const method = (spec) => {
    const inner = signature({ ...spec, name: spec.tool })
    return `    ${inner.replace(/^async def ([A-Za-z_][A-Za-z0-9_]*)\(/, 'async def $1(self, ').replace('(self, )', '(self)')}`
  }
  const protocols = servers.flatMap(([server, tools]) => {
    const body = tools.filter((t) => isUsableName(t.tool)).sort((a, b) => (a.tool < b.tool ? -1 : 1)).map(method)
  ...
  const lines = [
    ...
    `from __dsh__.tools import ToolCallError${servers.length === 0 ? '' : ', mcp'}${importable.map((spec) => `, ${spec.name}`).join('')}`,
    ...classes,
    ...mcpBlock,
    '',
    ...signatures,
