...
class Bridge:
    ...
    def call(self, name: str, args):
        ...
        # `_send` is async (it yields to the loop while the fd-3 buffer drains), but `call` stays sync so the binding can `return await bridge.call(...)` and hand the cell the Future itself to await — `await` on a coroutine that returned a Future would hand the cell a pending Future instead of its resolved value. A `call` frame is tiny, so fire it as a task: it runs immediately and only yields if the buffer is full, never blocking the caller.
        send = asyncio.create_task(self._send({"t": "call", "id": call_id, "shell": None if session is None else session.shell_id, "name": name, "args": args}))
