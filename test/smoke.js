...
import { PythonKernel, KERNEL_PY } from '../lib/kernel.js'
import { renderToolsSection, needsRestartNotice, specsKey, mcpPayload, toolSpecs, toolCallReply } from '../lib/index.js'
import { execFileSync } from 'node:child_process'
import './dispatch.js'

const KERNEL_PYTHON = execFileSync('uv', ['python', 'find', '--script', KERNEL_PY], { encoding: 'utf8' }).trim()
...
console.log('prompt:')
{
  ...
  const parses = (code) => {
    try {
      execFileSync(KERNEL_PYTHON, ['-c', 'import ast,sys; ast.parse(sys.stdin.read())'], { input: code, stdio: ['pipe', 'ignore', 'pipe'] })
      return true
    } catch { return false }
  }
  const cases = [
...
try {
  ...
  execFileSync(KERNEL_PYTHON, ['-c', 'import sys; compile(sys.stdin.read(), "<block>", "exec")'], { input: fence })
...
try {
  const listed = JSON.parse(execFileSync(KERNEL_PYTHON, ['-c', 'import json, keyword; print(json.dumps(keyword.kwlist + keyword.softkwlist))'], { encoding: 'utf8' }))
...
try {
  ...
  execFileSync(KERNEL_PYTHON, ['-c', 'import sys; compile(sys.stdin.read(), "<block>", "exec")'], { input: wideFence })
